...
// generator nomor surat masuk
function generate_nomor_masuk($pdo){
    $last = (int)$pdo->query('SELECT id FROM mail_in ORDER BY id DESC LIMIT 1')->fetchColumn();
    $next = $last + 1;
    $num  = str_pad($next, 3, '0', STR_PAD_LEFT);
    $year = date('Y');
    return "{$num}/SM/ADM/{$year}";
}

// tambah surat masuk (admin & pegawai boleh)
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $file = null;
    if (!empty($_FILES['file']['name'])) {
        $ext      = pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
        $safeName = 'mailin_'.time().'_'.mt_rand(1000,9999).'.'.$ext;
    ...
    if ($nomor === '') {
        $nomor = generate_nomor_masuk($pdo);
    }

    $stmt = $pdo->prepare(
        'INSERT INTO mail_in (no_agenda, nomor_surat, pengirim, perihal, tgl_surat, tgl_terima, file, status, created_by, created_at)
         VALUES (?,?,?,?,?,?,?,?,?,NOW())'
    );
    $stmt->execute([
        ...
        $_POST['tgl_terima'] ?: null,
        $file,
        'belum_dibaca',
        $user['id'],
    ]);

    header('Location: mail_in.php');
    exit;
}

// update status surat masuk (admin & pegawai boleh)
if ($action === 'update_status' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('UPDATE mail_in SET status = ? WHERE id = ?');
    $stmt->execute([$status, $id]);
    header('Location: mail_in.php');
    exit;
}

// hapus surat masuk (HANYA ADMIN)
if ($action === 'delete' && isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT file FROM mail_in WHERE id=?');
    $stmt->execute([$id]);
    $old = $stmt->fetch();
...
<main class="container">
  <div class="page-header">
    <div>
      <h2>Surat Masuk</h2>
      <p class="page-subtitle">Kelola arsip surat masuk dan statusnya (Belum dibaca → Proses → Selesai).</p>
    </div>
  </div>
